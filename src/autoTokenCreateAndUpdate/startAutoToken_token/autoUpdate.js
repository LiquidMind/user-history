...
db.query(
  `SELECT id,google_email, google_password FROM google_test WHERE saveToken = 1`,
  (err, result) => {
    if (err) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        const resEmail = Object.values(resObj)[0];
        ...
        const TOKEN_PATH = TOKEN_DIR + `${resEmail}.json`;
        function getNewToken(oauth2Client, callback) {
          ...
          rl.question(
            "Enter the code from the authorization page: ",
            (code) => {
              rl.close();
