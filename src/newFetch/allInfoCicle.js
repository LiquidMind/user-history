...
const apiKeys = [KEY, KEY2, KEY33]; // List of available API keys
...
function historyId(arrViewes) {
  ...
  function videoInfo(info) {
    ...
    const sqlQuery =
      "UPDATE videos_all SET viewes=?, oklike=?, lengthVideo=?, language=?, default_language=?, published_at=STR_TO_DATE(?, '%Y-%m-%dT%H:%i:%sZ'), channel_id=?, channel_title=?, dimension=?, definition=?, caption=?, favorite_count=?, comment_count=?, upload_status=?, privacy_status=?, license=?, public_stats_viewable=?, made_for_kids=? WHERE id=?";
    db.query(
      sqlQuery,
      [
        vieweVideo,
