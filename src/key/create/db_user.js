...
const downloadGoogleData = require("./downloadGoogleData");
  ...
  try {
    while (true) {
      const result = await new Promise((resolve, reject) => {
        const sqlQuery = `SELECT google_email, google_password, historyUpdatedAt FROM google_users WHERE statusKEY = 'proces' ORDER BY historyUpdatedAt ASC`;
      ...
      for (let i = 0; i < result.length; i++) {
        try {
          ...
          try {
            const sqlUpdate = `UPDATE google_users SET statusKEY = 'save' WHERE google_email = ?`;
            db.query(sqlUpdate, [resEmail], (err, result) => {
              if (err) {
                console.log(err);
              } else {
                console.log("Запис оновлено в базі даних");
              }
            });
          } catch (updateError) {
            console.log("Помилка при оновленні запису в БД:", updateError);
          }
        } catch (err) {
...
async function decryptKey(email, encryptedText) {
  try {
    ...
    return await downloadGoogleData(email, decrypted);
  } catch (err) {
    console.log(err);
    return false;
  }
